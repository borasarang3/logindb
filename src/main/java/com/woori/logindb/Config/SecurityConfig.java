...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        //2-1. 매핑권한(permitALL(), hasRole(), hasAnyRole())
        http.authorizeHttpRequests((auth)-> {
           //Controller의 모든 매핑에 대한 권한을 부여(**-모든매핑 대응)
            auth.requestMatchers("/").permitAll(); //시작페이지는 모든사용자 접근가능
            auth.requestMatchers("/result").permitAll(); //result매핑은 모든사용자 접근가능
            auth.requestMatchers("/user").hasAnyRole("USER", "ADMIN"); //USER사용자만 /user매핑으로 접근가능
